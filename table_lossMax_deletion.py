...
## Loss-maximizing Deletion
def k_lossMax_compare_robust(points_index: np.ndarray, complete: np.ndarray) -> tuple[np.ndarray, float, np.ndarray]:
    """
    ...
    for round in range(3):
        ...
        for i in range(10):
            ...
            points_left_points, points_deleted_points = loss_max_deletion(points_coreset, z, complete)
            loss_best = float("inf")
            for d in range(10):
...
def k_lossMax_compare_GMM(points_index: np.ndarray, complete: np.ndarray) -> tuple[np.ndarray, float]:
    """
    ...
    for round in range(3):
        ...
        for i in range(10):
            ...
            points_left_points, points_deleted_points = loss_max_deletion(points_gmm_array, z, complete)
            loss_best = float("inf")
            for d in range(10):
                random.seed(7*d*d+6*d+12)
                best_answers = GMM(points_left_points, k, complete)
...
def k_lossMax_compare_fault(a: int, complete: np.ndarray, edges: np.ndarray) -> tuple[np.ndarray, float]:
    """
    ...
    for round in range(3):
        ...
        points_left_points, points_deleted_points = loss_max_deletion(points_centers_array, z, complete)
        loss_best = float("inf")
        for d in range(10):
            random.seed(7*d*d+6*d+12)
            best_answers = GMM(points_left_points, k, complete)
...
def main():
    ...
    ## 5. Fix $k$ Loss-maximizing Deletion Implementation
    index = np.arange(1000)
    a = 2
    times = []
    all_size_coreset = []

    adult_robust, time_temp, size_coreset_array = k_lossMax_compare_robust(index, adult_complete)
    np.save("results_1/adult_lossMax_robust.npy", adult_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    adult_gmm, time_temp = k_lossMax_compare_GMM(index, adult_complete)
    np.save("results_1/adult_lossMax_gmm.npy", adult_gmm)
    times.append(time_temp)

    adult_fault, time_temp = k_lossMax_compare_fault(a, adult_complete, adult_edges)
    np.save("results_1/adult_lossMax_fault.npy", adult_fault)
    times.append(time_temp)

    CelebA_robust, time_temp, size_coreset_array = k_lossMax_compare_robust(index, CelebA_complete)
    np.save("results_1/CelebA_lossMax_robust.npy", CelebA_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    CelebA_gmm, time_temp = k_lossMax_compare_GMM(index, CelebA_complete)
    np.save("results_1/CelebA_lossMax_gmm.npy", CelebA_gmm)
    times.append(time_temp)

    CelebA_fault, time_temp = k_lossMax_compare_fault(a, CelebA_complete, CelebA_edges)
    np.save("results_1/CelebA_lossMax_fault.npy", CelebA_fault)
    times.append(time_temp)

    Gaussian_blob_robust, time_temp, size_coreset_array = k_lossMax_compare_robust(index, Gaussian_blob_complete)
    np.save("results_1/Gaussian_blob_lossMax_robust.npy", Gaussian_blob_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    Gaussian_blob_gmm, time_temp = k_lossMax_compare_GMM(index, Gaussian_blob_complete)
    np.save("results_1/Gaussian_blob_lossMax_gmm.npy", Gaussian_blob_gmm)
    times.append(time_temp)

    Gaussian_blob_fault, time_temp = k_lossMax_compare_fault(a, Gaussian_blob_complete, Gaussian_blob_edges)
    np.save("results_1/Gaussian_blob_lossMax_fault.npy", Gaussian_blob_fault)
    times.append(time_temp)

    glove_robust, time_temp, size_coreset_array = k_lossMax_compare_robust(index, glove_complete)
    np.save("results_1/glove_lossMax_robust.npy", glove_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    glove_gmm, time_temp = k_lossMax_compare_GMM(index, glove_complete)
    np.save("results_1/glove_lossMax_gmm.npy", glove_gmm)
    times.append(time_temp)

    glove_fault, time_temp = k_lossMax_compare_fault(a, glove_complete, glove_edges)
    np.save("results_1/glove_lossMax_fault.npy", glove_fault)
    times.append(time_temp)

    movielens_robust, time_temp, size_coreset_array = k_lossMax_compare_robust(index, movielens_complete)
    np.save("results_1/movielens_lossMax_robust.npy", movielens_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    movielens_gmm, time_temp = k_lossMax_compare_GMM(index, movielens_complete)
    np.save("results_1/movielens_lossMax_gmm.npy", movielens_gmm)
    times.append(time_temp)

    movielens_fault, time_temp = k_lossMax_compare_fault(a, movielens_complete, movielens_edges)
    np.save("results_1/movielens_lossMax_fault.npy", movielens_fault)
    times.append(time_temp)

    times = np.array(times)
    all_size_coreset = np.array(all_size_coreset)
    np.save("results_1/time_k_lossMax_deletion.npy", times)
    np.save("results_1/size_coreset_k_lossMax_deletion.npy", all_size_coreset)
