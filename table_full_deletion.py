...
## Full-knowledge Deletion
def full_compare_robust(points_index: np.ndarray, complete: np.ndarray) -> tuple[np.ndarray, float, np.ndarray]:
    """
...
def full_compare_GMM(points_index: np.ndarray, complete: np.ndarray) -> tuple[np.ndarray, float]:
    """
...
def full_compare_fault(a: int, complete: np.ndarray, edges: np.ndarray) -> tuple[np.ndarray, float]:
    """
...
def main():
    ...
    ## 1. Full Deletion Implementation
    index = np.arange(1000)
    a = 2
    times = []
    all_size_coreset = []

    adult_robust, time_temp, size_coreset_array = full_compare_robust(index, adult_complete)
    np.save("results_1/adult_full_robust.npy", adult_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    adult_gmm, time_temp =(index, adult_complete)
    np.save("results_1/adult_full_gmm.npy", adult_gmm)
    times.append(time_temp)

    adult_fault, time_temp = full_compare_fault(a, adult_complete, adult_edges)
    np.save("results_1/adult_full_fault.npy", adult_fault)
    times.append(time_temp)

    CelebA_robust, time_temp, size_coreset_array = full_compare_robust(index, CelebA_complete)
    np.save("results_1/CelebA_full_robust.npy", CelebA_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    CelebA_gmm, time_temp =(index, CelebA_complete)
    np.save("results_1/CelebA_full_gmm.npy", CelebA_gmm)
    times.append(time_temp)

    CelebA_fault, time_temp = full_compare_fault(a, CelebA_complete, CelebA_edges)
    np.save("results_1/CelebA_full_fault.npy", CelebA_fault)
    times.append(time_temp)

    Gaussian_blob_robust, time_temp, size_coreset_array = full_compare_robust(index, Gaussian_blob_complete)
    np.save("results_1/Gaussian_blob_full_robust.npy", Gaussian_blob_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    Gaussian_blob_gmm, time_temp =(index, Gaussian_blob_complete)
    np.save("results_1/Gaussian_blob_full_gmm.npy", Gaussian_blob_gmm)
    times.append(time_temp)

    Gaussian_blob_fault, time_temp = full_compare_fault(a, Gaussian_blob_complete, Gaussian_blob_edges)
    np.save("results_1/Gaussian_blob_full_fault.npy", Gaussian_blob_fault)
    times.append(time_temp)

    glove_robust, time_temp, size_coreset_array = full_compare_robust(index, glove_complete)
    np.save("results_1/glove_full_robust.npy", glove_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    glove_gmm, time_temp =(index, glove_complete)
    np.save("results_1/glove_full_gmm.npy", glove_gmm)
    times.append(time_temp)

    glove_fault, time_temp = full_compare_fault(a, glove_complete, glove_edges)
    np.save("results_1/glove_full_fault.npy", glove_fault)
    times.append(time_temp)

    movielens_robust, time_temp, size_coreset_array = full_compare_robust(index, movielens_complete)
    np.save("results_1/movielens_full_robust.npy", movielens_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    movielens_gmm, time_temp =(index, movielens_complete)
    np.save("results_1/movielens_full_gmm.npy", movielens_gmm)
    times.append(time_temp)

    movielens_fault, time_temp = full_compare_fault(a, movielens_complete, movielens_edges)
    np.save("results_1/movielens_full_fault.npy", movielens_fault)
    times.append(time_temp)

    times = np.array(times)
    all_size_coreset = np.array(all_size_coreset)
    np.save("results_1/time_full_deletion.npy", times)
    np.save("results_1/size_coreset_full_deletion.npy", all_size_coreset)
