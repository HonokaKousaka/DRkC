...
## Fix k WB-NN
def k_WBNN_compare_robust(points_index: np.ndarray, k: int, complete: np.ndarray) -> tuple[np.ndarray, float, np.ndarray]:
    """
    Returns the ratio of the loss caused by our algorithm to the optimal loss after WBNN deletion with k immutable.
    Args: 
        points_index (np.ndarray): The indexes of data
        k (int): A decimal integer, the number of centers
        complete (np.ndarray): The adjacency matrix of n points
...
def k_WBNN_compare_GMM(points_index: np.ndarray, k: int, complete: np.ndarray) -> tuple[np.ndarray, float]:
    """
    Returns the ratio of the loss caused by GMM to the optimal loss after WBNN deletion with k immutable.
    Args: 
        points_index (np.ndarray): The indexes of data
        k (int): A decimal integer, the number of centers
        complete (np.ndarray): The adjacency matrix of n points
...
def k_WBNN_compare_fault(k: int, a: int, complete: np.ndarray, edges: np.ndarray) -> tuple[np.ndarray, float]:
    """
    Returns the ratio of the loss caused by Fault Tolerant algorithm to the optimal loss after WBNN deletion with k immutable.
    Args: 
        k (int): A decimal integer, the number of centers
...
def main():
    ...
    adult_robust, time_temp, size_coreset_array = k_WBNN_compare_robust(index, k, adult_complete)
    np.save("results_2/adult_complete_robust.npy", adult_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    adult_gmm, time_temp = k_WBNN_compare_GMM(index, k, adult_complete)
    np.save("results_2/adult_complete_gmm.npy", adult_gmm)
    times.append(time_temp)

    adult_fault, time_temp = k_WBNN_compare_fault(k, a, adult_complete, adult_edges)
    np.save("results_2/adult_complete_fault.npy", adult_fault)
    times.append(time_temp)

    CelebA_robust, time_temp, size_coreset_array = k_WBNN_compare_robust(index, k, CelebA_complete)
    np.save("results_2/CelebA_complete_robust.npy", CelebA_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    CelebA_gmm, time_temp = k_WBNN_compare_GMM(index, k, CelebA_complete)
    np.save("results_2/CelebA_complete_gmm.npy", CelebA_gmm)
    times.append(time_temp)

    CelebA_fault, time_temp = k_WBNN_compare_fault(k, a, CelebA_complete, CelebA_edges)
    np.save("results_2/CelebA_complete_fault.npy", CelebA_fault)
    times.append(time_temp)

    Gaussian_blob_robust, time_temp, size_coreset_array = k_WBNN_compare_robust(index, k, Gaussian_blob_complete)
    np.save("results_2/Gaussian_blob_complete_robust.npy", Gaussian_blob_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    Gaussian_blob_gmm, time_temp = k_WBNN_compare_GMM(index, k, Gaussian_blob_complete)
    np.save("results_2/Gaussian_blob_complete_gmm.npy", Gaussian_blob_gmm)
    times.append(time_temp)

    Gaussian_blob_fault, time_temp = k_WBNN_compare_fault(k, a, Gaussian_blob_complete, Gaussian_blob_edges)
    np.save("results_2/Gaussian_blob_complete_fault.npy", Gaussian_blob_fault)
    times.append(time_temp)

    glove_robust, time_temp, size_coreset_array = k_WBNN_compare_robust(index, k, glove_complete)
    np.save("results_2/glove_complete_robust.npy", glove_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    glove_gmm, time_temp = k_WBNN_compare_GMM(index, k, glove_complete)
    np.save("results_2/glove_complete_gmm.npy", glove_gmm)
    times.append(time_temp)

    glove_fault, time_temp = k_WBNN_compare_fault(k, a, glove_complete, glove_edges)
    np.save("results_2/glove_complete_fault.npy", glove_fault)
    times.append(time_temp)

    movielens_robust, time_temp, size_coreset_array = k_WBNN_compare_robust(index, k, movielens_complete)
    np.save("results_2/movielens_complete_robust.npy", movielens_robust)
    times.append(time_temp)
    all_size_coreset.append(size_coreset_array)

    movielens_gmm, time_temp = k_WBNN_compare_GMM(index, k, movielens_complete)
    np.save("results_2/movielens_complete_gmm.npy", movielens_gmm)
    times.append(time_temp)

    movielens_fault, time_temp = k_WBNN_compare_fault(k, a, movielens_complete, movielens_edges)
